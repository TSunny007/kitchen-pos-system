...
-- Tighten orders.status to the values the app actually uses.
alter table public.orders drop constraint if exists orders_status_check;
alter table public.orders add constraint orders_status_check
  check (status in ('new', 'in_progress', 'completed', 'cancelled'));

-- Tighten order_items.status: drop legacy 'picked_up' (0 rows use it;
-- superseded by order-level completion instead of a separate item state).
