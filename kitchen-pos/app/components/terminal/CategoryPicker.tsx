...
export default function CategoryPicker({
  ...
  const [togglingLabelCategoryId, setTogglingLabelCategoryId] = useState<number | null>(null);
  ...
  const handleToggleRequiresLabel = async (category: Category, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!onToggleCategoryRequiresLabel) return;

    setTogglingLabelCategoryId(category.id);
    try {
      await onToggleCategoryRequiresLabel(category.id, !category.requires_label);
    } finally {
      setTogglingLabelCategoryId(null);
    }
  };
            ...
            {filteredCategories.length > 0 ? (
              filteredCategories.map((category) => (
                ...
                >
                  ...
                  {/* Requires Cup Label Toggle */}
                  {onToggleCategoryRequiresLabel && (
                    <button
                      type="button"
                      onClick={(e) => handleToggleRequiresLabel(category, e)}
                      disabled={togglingLabelCategoryId === category.id}
                      className={`shrink-0 rounded p-1 transition-colors ${
                        category.requires_label
                          ? "text-primary hover:bg-primary-container"
                          : "text-on-surface-variant hover:bg-surface-container-high"
                      } disabled:opacity-50`}
                      title={
                        category.requires_label
                          ? "Prints a cup label on start — click to stop"
                          : "Click to print a cup label when an item starts"
                      }
                    >
                      {togglingLabelCategoryId === category.id ? (
                        ...
                        <svg
                          className="h-4 w-4"
                          fill={category.requires_label ? "currentColor" : "none"}
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6M9 3h6a2 2 0 012 2v2H7V5a2 2 0 012-2zM7 7h10a2 2 0 012 2v6a2 2 0 01-2 2h-1v2a1 1 0 01-1 1H9a1 1 0 01-1-1v-2H7a2 2 0 01-2-2V9a2 2 0 012-2z" />
                        </svg>
                      )}
                    </button>
                  )}
